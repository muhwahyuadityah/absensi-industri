<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ auth()->user()->hasRole('Manager') ? route('manager.users.store') : route('admin.users.store') }}" x-data="userForm()">
                        @csrf
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="email" name="email" value="{{ old('email') }}" required
                                           class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <p class="text-xs text-gray-500 mt-1">Email untuk login & verifikasi akun</p>
                                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select name="employee_type" x-model="employeeType" required
                                        ...
                                        <option value="AREA_BASED" {{ old('employee_type') == 'AREA_BASED' ? 'selected' : '' }}>
                                            Area Based (Produksi)
                                        </option>
                                        <option value="NON_AREA" {{ old('employee_type') == 'NON_AREA' ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Shift dapat diubah sewaktu-waktu</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                            <h4 class="font-bold text-blue-900 mb-2">ℹ️ Informasi Password</h4>
                            <ul class="text-sm text-blue-800 space-y-1">
                                <li>✓ User akan menerima email verifikasi di alamat email yang didaftarkan</li>
                                <li>✓ Email berisi link untuk set password & aktivasi akun</li>
                                <li>✓ Link verifikasi berlaku selama 24 jam</li>
                                <li>✓ Setelah verifikasi, user dapat login dengan email & password yang dibuat</li>
                            </ul>
                        </div>
    ...
    <script>
        function userForm() {
            return {
                employeeType: '{{ old("employee_type") }}' || ''
            }
        }
    </script>
</x-app-layout>